class_name GatherFood2 extends Objective
...
func _init(a: Agent) -> void:
	...
	add_precondition(GotoFarm2.new(agent,  farm_to_gather_from), "gather_food")
	precondition("gather_food").prefered_next_task = self
